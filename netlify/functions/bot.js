...
bot.start(async (ctx) => {
  ...
  await ctx.replyWithMarkdown(
    ...
    {
      reply_markup: {
        keyboard: [['🎫 Получить QR для визита'], ['⭐ Мой статус', '📋 История визитов']],
...
bot.hears('🎫 Получить QR для визита', async (ctx) => {
  ...
  await ctx.replyWithPhoto(
    { source: qrBuffer },
    { caption: `🎫 *Твой QR-код для визита*\n\nПокажи этот код администратору.\nКод действителен для одного визита.`, parse_mode: 'Markdown' }
  );
});

bot.hears('⭐ Мой статус', async (ctx) => {
  const { data: guest } = await supabase
    .from('guests').select('visit_count, first_name').eq('telegram_id', String(ctx.from.id)).single();
...
bot.hears('📋 История визитов', async (ctx) => {
  const { data: visits } = await supabase
    .from('visits').select('visited_at, restaurants(name)')
    .eq('telegram_id', String(ctx.from.id)).order('visited_at', { ascending: false }).limit(10);
  ...
  const lines = visits.map((v, i) =>
    `${i + 1}. ${v.restaurants?.name || 'Ресторан'} — ${new Date(v.visited_at).toLocaleDateString('ru-RU')}`
  );
